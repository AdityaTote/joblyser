...
func New(ctx context.Context, cfg config.Config) (*Database, error) {
	encodePassword := url.QueryEscape(cfg.DbPassword)

	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=%s sslmode=%s", cfg.DbHost, cfg.DbUser, encodePassword, cfg.DbName, cfg.DbPort, "disable")
