...
import { ArrowLeft, Globe, Lock, Mail, Zap } from "lucide-react";
import { useGoogleUrl, useSignIn } from "@/hooks/queries/useAuth";
...
export default function SignInPage() {
  ...
  const isSubmitting = signInMutation.isPending || googleMutation.isPending;
  const errorMessage =
    signInMutation.error?.message || googleMutation.error?.message;
      ...
      <div className="flex items-start justify-center px-4 pb-10 sm:px-6">
        <section className="w-full max-w-[320px] overflow-hidden rounded-[18px] border border-[#d6d7dc] bg-[#f7f7f8] shadow-[0_5px_16px_rgb(19_20_28/7%)]">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4 text-left">
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <label
                    htmlFor="password"
                    className="block text-xs font-semibold text-[#23252d]"
                  >
                    Password
                  </label>
                  <button
                    type="button"
                    className="text-[11px] font-medium text-[#9a9ea9]"
                  >
                    Forgot password?
                  </button>
                </div>
          ...
          <div className="border-t border-[#dfdfe4] bg-[#f3f3f5] px-3 py-4 text-center text-sm text-[#888c96]">
            ...
            <Link href="/signup" className="font-semibold text-[#1a1b22]">
              Sign up
            </Link>
          </div>
        </section>
